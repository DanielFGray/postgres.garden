 ...
 */
const createProviderLayer = (provider: ProviderName, config: ProviderConfig) => {
  switch (provider) {
    case "openai":
...
export const generateText = (options: {
  ...
  class ProviderModels extends LayerMap.Service<ProviderModels>()(`${provider}Models`, {
 ...
 * ```
 */
export const generateTextFromEnv = (prompt: string, model: string) =>
  Effect.gen(function* () {
