...
export const PROVIDER_METADATA = {
  ...
  anthropic: {
    ...
    defaultModels: ["claude-3-5-sonnet-20241022", "claude-3-opus-20240229"] as const,
    requiresApiKey: true,
  },
  google: {
...
export const createOpenAiLayer = (config: ProviderConfig) => {
  const apiKey = typeof config.apiKey === "string" ? Redacted.make(config.apiKey) : config.apiKey;
 ...
 */
export const getLanguageModel = (provider: ProviderName, modelName: string) => {
  switch (provider) {
    case "openai":
 ...
 */
export type AnyProviderClient = OpenAiClient | AnthropicClient | GoogleClient | OpenRouterClient;
