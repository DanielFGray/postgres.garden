...
export async function staticPlugin<const Prefix extends string = '/prefix'>({
    ...
    if (alwaysStatic) {
        ...
        if (files.length <= staticLimit)
            for (const absolutePath of files) {
                ...
                // PATCHED: Skip only the root index.html - it's served by catch-all with SSR data injection
                // Other HTML files (webviews, workers) must be served as static assets
    ...
    if (
        ...
        // PATCHED: Removed HTML file import logic to fix bun build --compile issues
        // HTML files will be served via the wildcard route below
