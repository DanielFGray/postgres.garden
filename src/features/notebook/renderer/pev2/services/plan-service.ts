...
export class PlanService {
  ...
  public createPlan(planName: string, planContent: IPlanContent, planQuery: string): IPlan {
    ...
    this.flat = this.flat.concat(_.flattenDeep(this.recurse([plan.content.Plan as Node])));
    ...
    _.each(node[NodeProp.PLANS], (child) => {
      ...
      if (
        ...
        child[NodeProp.WORKERS_LAUNCHED_BY_GATHER] =
          node[NodeProp.WORKERS_LAUNCHED] || node[NodeProp.WORKERS_LAUNCHED_BY_GATHER];
      }
      if (this.isCTE(child)) {
  ...
  public calculateActuals(node: Node) {
    if (!_.isUndefined(node[NodeProp.ACTUAL_TOTAL_TIME])) {
      ...
      node[NodeProp.ACTUAL_TOTAL_TIME] =
        ((node[NodeProp.ACTUAL_TOTAL_TIME] as number) * (node[NodeProp.ACTUAL_LOOPS] as number)) /
        workers;
      node[NodeProp.ACTUAL_STARTUP_TIME] =
        ((node[NodeProp.ACTUAL_STARTUP_TIME] as number) * (node[NodeProp.ACTUAL_LOOPS] as number)) /
        workers;
      node[NodeProp.EXCLUSIVE_DURATION] = node[NodeProp.ACTUAL_TOTAL_TIME];

      const duration =
        (node[NodeProp.EXCLUSIVE_DURATION] as number) - this.childrenDuration(node, 0);
    ...
    _.each(node[NodeProp.PLANS], (subPlan) => {
      if (subPlan[NodeProp.TOTAL_COST]) {
        node[NodeProp.EXCLUSIVE_COST] =
          (node[NodeProp.EXCLUSIVE_COST] as number) - (subPlan[NodeProp.TOTAL_COST] as number);
      }
    });
  ...
  public fixCteScansDuration(plan: IPlan) {
    ...
    _.each(plan.ctes, (cte) => {
      ...
      const cteScans = _.filter(
        this.flat,
        (node) => `CTE ${node[NodeProp.CTE_NAME]}` == cte[NodeProp.SUBPLAN_NAME],
      );

      // Sum of exclusive time for the CTE Scans
      const sumScansDuration = _.sumBy(cteScans, (node) => node[NodeProp.EXCLUSIVE_DURATION]);
  ...
  public fixInitPlanUsageDuration(plan: IPlan) {
    ...
    _.each(initPlans, (subPlan) => {
      ...
      _.each(
        _.filter(this.flat, (node) => node[NodeProp.PARENT_RELATIONSHIP] != "InitPlan"),
        (node) => {
          _.each(node, (value) => {
            ...
            // matching exactly)
            const matches = new RegExp(`.*${name.replace(/[^a-zA-Z0-9]/g, "\\$&")}[0-9]?`).exec(
              value,
            );
            if (matches) {
              node[NodeProp.EXCLUSIVE_DURATION] -= subPlan[NodeProp.ACTUAL_TOTAL_TIME] || 0;
  ...
  public childrenDuration(node: Node, duration: number) {
    _.each(node[NodeProp.PLANS], (child) => {
      ...
      if (
        child[NodeProp.PARENT_RELATIONSHIP] !== "InitPlan" ||
        (child[NodeProp.PARENT_RELATIONSHIP] == "InitPlan" && node[NodeProp.NODE_TYPE] == "Result")
      ) {
  ...
  public splitIntoLines(text: string): string[] {
    ...
    _.each(lines, (line: string) => {
      ...
      if (previousLine && countChar(previousLine, /\)/g) != countChar(previousLine, /\(/g)) {
      ...
      if (emptyLineMatches || headerMatches) {
        ...
        if (
          ...
          newNode[NodeProp.PLAN_ROWS] = parseInt(
            nodeMatches[NodeMatch.EstimatedRows] || nodeMatches[NodeMatch.EstimatedRows2],
            0,
          );
          newNode[NodeProp.PLAN_WIDTH] = parseInt(
            nodeMatches[NodeMatch.EstimatedRowWidth] || nodeMatches[NodeMatch.EstimatedRowWidth2],
            0,
          );
        }
        if (
          (nodeMatches[NodeMatch.ActualTimeFirst1] && nodeMatches[NodeMatch.ActualTimeLast1]) ||
          (nodeMatches[NodeMatch.ActualTimeFirst2] && nodeMatches[NodeMatch.ActualTimeLast2])
        ) {
          newNode[NodeProp.ACTUAL_STARTUP_TIME] = parseFloat(
            nodeMatches[NodeMatch.ActualTimeFirst1] || nodeMatches[NodeMatch.ActualTimeFirst2],
          );
          newNode[NodeProp.ACTUAL_TOTAL_TIME] = parseFloat(
            nodeMatches[NodeMatch.ActualTimeLast1] || nodeMatches[NodeMatch.ActualTimeLast2],
          );
        }

        if (
          (nodeMatches[NodeMatch.ActualRows1] && nodeMatches[NodeMatch.ActualLoops1]) ||
          (nodeMatches[NodeMatch.ActualRows2] && nodeMatches[NodeMatch.ActualLoops2])
        ) {
          const actual_rows =
            nodeMatches[NodeMatch.ActualRows1] || nodeMatches[NodeMatch.ActualRows2];
          ...
          newNode[NodeProp.ACTUAL_LOOPS] = parseInt(
            nodeMatches[NodeMatch.ActualLoops1] || nodeMatches[NodeMatch.ActualLoops2],
            0,
          );
        }
        ...
        if (
          ...
          worker[NodeProp.ACTUAL_ROWS] = parseInt(workerMatches[WorkerMatch.ActualRows], 0);
          worker[NodeProp.ACTUAL_LOOPS] = parseInt(workerMatches[WorkerMatch.ActualLoops], 0);
        }
        ...
        // extra info
        const info = workerMatches[WorkerMatch.Extra].split(/: (.+)/).filter((x) => x);
  ...
  private parseIOTimings(text: string, el: Node): boolean {
    ...
    const scopeRegex = /\b(shared\/local|shared|local|temp)((?:\s+(?:read|write)=\d+(?:\.\d+)?)+)/g;
    ...
    while ((scopeMatch = scopeRegex.exec(text)) !== null) {
      ...
      while ((operationMatch = operationRegex.exec(operationsString)) !== null) {
        entry[operationMatch[1] as "read" | "write"] = parseFloat(operationMatch[2]);
      }
    ...
    while ((operationMatch = operationRegex.exec(rest)) !== null) {
      unscoped[operationMatch[1] as "read" | "write"] = parseFloat(operationMatch[2]);
      found = true;
    }
    ...
    results.forEach((result) => {
      ["read", "write"].forEach((operation) => {
        ...
        if (result.scope && result.scope != "shared/local") {
          prop = (_.upperCase(result.scope) + "_" + prop) as keyof typeof NodeProp;
        }
    ...
    _.each(properties, (property) => {
      const sum = Number(
        _.sumBy(
          ...
          _.filter(node[NodeProp.PLANS], (child: Node) => !child[NodeProp.SUBPLAN_NAME]),
          (child: Node) => {
      ...
      node[nodeProp] = Number(((node[NodeProp[property]] as number) - sum).toFixed(3));
    });
  }
  ...
  private calculateIoTimingsAverage(node: Node) {
    ...
    operations.forEach((operation, index) => {
      ["exclusive_", ""].forEach((prefix) => {
        ...
        node[NodeProp[sumTimeProp] as unknown as keyof typeof Node] = Number(time.toFixed(3));
        if (time) {
          node[NodeProp[speedProp] as unknown as keyof typeof Node] = Number(
            (buffers / (time / 1000)).toFixed(3),
          );
