...
import { BufferLocation, NodeProp, EstimateDirection, HighlightType } from "@/enums";
...
export default function useNode(node: Node, viewOptions: ViewOptions) {
  ...
  const estimateFactorPercent = computed((): number => {
    switch (node[NodeProp.PLANNER_ESTIMATE_FACTOR]) {
      ...
      default:
        return (
          ((node[NodeProp.PLANNER_ESTIMATE_FACTOR] || 0) / store.stats.maxEstimateFactor) * 100
        );
    }
  });
  ...
  const tempReadPercent = computed((): number => {
    return (
      (node[NodeProp.EXCLUSIVE_TEMP_READ_BLOCKS] / store.stats.maxBlocks?.[BufferLocation.temp]) *
      100
    );
  });
  ...
  const localHitPercent = computed((): number => {
    return (
      (node[NodeProp.EXCLUSIVE_LOCAL_HIT_BLOCKS] / store.stats.maxBlocks?.[BufferLocation.local]) *
      100
    );
  });
  ...
  const localReadPercent = computed((): number => {
    return (
      (node[NodeProp.EXCLUSIVE_LOCAL_READ_BLOCKS] / store.stats.maxBlocks?.[BufferLocation.local]) *
      100
    );
  });
  ...
  const buffersByLocationTooltip = computed(() => (location: BufferLocation): string => {
    let text = "";
    let hit;
    let read;
    let written;
    ...
    text += hit ? '<tr><td>Hit:</td><td class="text-end">' + blocks(hit, true) + "</td></tr>" : "";
    text += read
      ? '<tr><td>Read:</td><td class="text-end">' + blocks(read, true) + "</td></tr>"
      : "";
    text += dirtied
      ? '<tr><td>Dirtied:</td><td class="text-end">' + blocks(dirtied, true) + "</td></tr>"
      : "";
    text += written
      ? '<tr><td>Written:</td><td class="text-end">' + blocks(written, true) + "</td></tr>"
      : "";
    text += "</table>";
