...
import type { Node, IPlan, IPlanContent, IPlanStats, IBlocksStats, Settings } from "@/interfaces";
...
function flattenPlan(root: Node, nodeById: FlattenedNodeMap): FlattenedPlanNode[] {
  ...
  const visit = (node: Node, path: number[], branches: boolean[], level: number): void => {
