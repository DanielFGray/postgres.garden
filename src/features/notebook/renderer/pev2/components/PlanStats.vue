...
function averageIO(node: Node) {
  ...
  const r = [];
  if (readAverage) {
    r.push(`read=~${formatNodeProp(NodeProp.AVERAGE_SUM_IO_READ_SPEED, readAverage)}`);
  }
  if (writeAverage) {
    r.push(`write=~${formatNodeProp(NodeProp.AVERAGE_SUM_IO_WRITE_SPEED, writeAverage)}`);
  }
  return r.join(", ");
}
...
<template>
  ...
  >
    <div class="d-inline-block px-2">
      ...
      <template v-else>
        <span class="stat-value" v-html="duration(store.stats.executionTime)"></span>
      </template>
    </div>
        ...
        <span class="stat-value">
          <span
            :class="
              'mb-0 p-0 px-1 alert ' +
              planningTimeClass(
                (store.stats.planningTime / (store.stats.executionTime as number)) * 100,
              )
            "
    ...
    >
      ...
      <span class="stat-value">
        <span
          :class="
            'mb-0 p-0 px-1 alert ' +
            planningTimeClass((store.stats.jitTime / store.stats.executionTime) * 100)
          "
        ...
        >
          <FontAwesomeIcon :icon="faCaretDown" class="text-body-tertiary"></FontAwesomeIcon>
        </button>
        <div class="stat-dropdown-container text-start" v-if="showJitDetails">
          <div>
            <JitDetails :jit="store.plan?.content.JIT" v-if="store.plan?.content.JIT" />
          </div>
        </div>
      </span>
    </div>
    ...
    >
      ...
      <span class="stat-value">
        <span
          :class="'mb-0 p-0 px-1 alert ' + durationClass(totalTriggerDurationPercent)"
      ...
      <div class="stat-dropdown-container text-start" v-if="showTriggers">
        <button class="btn btn-xs btn-close float-end" v-on:click="showTriggers = false"></button>
        <h3>Triggers</h3>
        <div v-for="(trigger, index) in store.stats.triggers" :key="index">
          ...
          <span class="float-end">
            <span
              :class="'p-0 px-1 alert ' + durationClass(triggerDurationPercent(trigger))"
              v-html="duration(trigger.Time)"
            ></span>
            | {{ triggerDurationPercent(trigger) }}<span class="text-body-tertiary">%</span>
          </span>
          <br />
          ...
          <hr
            v-if="store.stats.triggers && index != store.stats.triggers.length - 1"
            class="my-2"
          />
        </div>
      </div>
    </div>
    <div class="d-inline-block border-start px-2 position-relative" v-if="store.stats.settings">
      <span class="stat-label"
        >Settings:
        <span class="badge bg-secondary">{{ _.keys(store.stats.settings).length }}</span></span
      >
      <button
      ...
      <div class="stat-dropdown-container text-start" v-if="showSettings">
        <button class="btn btn-xs btn-close float-end" v-on:click="showSettings = false"></button>
        <h3>PG Settings</h3>
        <em class="text-body-tertiary d-block pb-2">
          Configuration parameters affecting query planning with value different from the built-in
          default value.
        </em>
    ...
    >
      ...
      ></FontAwesomeIcon>
      <button @click.prevent="showIO = !showIO" class="bg-transparent border-0 p-0 m-0 ps-1">
        <FontAwesomeIcon :icon="faCaretDown" class="text-body-tertiary"></FontAwesomeIcon>
      </button>
      <div class="stat-dropdown-container text-start" v-if="showIO">
        <button class="btn btn-xs btn-close float-end" v-on:click="showIO = false"></button>
