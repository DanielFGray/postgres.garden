...
const connector = props.isSubplan ? (isBranch ? `${SPACE} ` : VERTICAL) : isBranch ? LAST : BRANCH;
  ...
  <span v-if="branches.length" class="plan-tree">
    ...
    ><span
      :class="
        ...
        (highlightedNodeId == row.node.nodeId || highlightedNodeLevel == row.level - 1)
          ? 'text-body-emphasis'
          : highlightedNodeId
            ? 'text-body-tertiary'
            : 'text-body-secondary'
      "
      >{{ highlightedNodeId == row.node.nodeId ? `${SPACE}▶` : connector }}</span
    >
  </span>
</template>
