...
const readTimeProp = (exclusivePrefix + scope + "IO_READ_TIME") as keyof typeof NodeProp;
const readSpeedProp = (exclusivePrefix +
  "AVERAGE_" +
  scope +
  "IO_READ_SPEED") as keyof typeof NodeProp;
const readBlocksProp = (exclusivePrefix + scope + "READ_BLOCKS") as keyof typeof NodeProp;
const writeTimeProp = (exclusivePrefix + scope + "IO_WRITE_TIME") as keyof typeof NodeProp;
const writeSpeedProp = (exclusivePrefix +
  "AVERAGE_" +
  scope +
  "IO_WRITE_SPEED") as keyof typeof NodeProp;
const writtenBlocksProp = (exclusivePrefix + scope + "WRITTEN_BLOCKS") as keyof typeof NodeProp;
</script>
...
<template>
  <tr>
    ...
    <td class="text-end" v-if="node[NodeProp[readTimeProp]]">
      ...
      <br />
      <small>{{ blocksAsBytes(node[NodeProp[readBlocksProp]] as number) }}</small>
      <br />
    ...
    <td class="text-end" v-if="node[NodeProp[writeTimeProp]]">
      ...
      <br />
      <small>{{ blocksAsBytes(node[NodeProp[writtenBlocksProp]] as number) }}</small>
      <br />
