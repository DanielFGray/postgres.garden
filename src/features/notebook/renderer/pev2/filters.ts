...
export function sortKeys(sort: string[], presort: string[] | undefined): string {
  return _.map(sort, (v) => {
    ...
    if (presort) {
      result +=
        presort.indexOf(v) !== -1 ? '&nbsp;<span class="text-body-tertiary">(presort)</span>' : "";
    }
    return result;
  }).join(", ");
}
...
export function list(value: string[] | string): string {
  ...
  return '<ul class="list-unstyled mb-0">' + compiled({ lines: value }) + "</ul>";
}

function sortGroups(value: string): string {
  const app = createApp(SortGroup, { sortGroup: value }).mount(document.createElement("div"));
...
function jit(value: JIT): string {
  const app = createApp(JitDetails, { jit: value }).mount(document.createElement("div"));
  return app.$el.outerHTML;
}
