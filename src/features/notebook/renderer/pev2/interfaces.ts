...
export interface IPlanContent {
  ...
  JIT?: JIT;
  "Query Text"?: string;
  [k: string]: Node | number | string | IBlocksStats | ITrigger[] | JIT | undefined;
}
...
export class Node {
  ...
  constructor(type?: string) {
    ...
    const functionRegex = /^(Function\sScan)\son\s(\S+)(?:\s+(\S+))?$/.exec(type);
    enum SubqueryMatch {
      NodeType = 1,
      Alias,
    }
    ...
    if (scanAndOperationsRegex) {
      this[NodeProp.NODE_TYPE] = scanAndOperationsRegex[ScanAndOperationMatch.NodeType];
      this[NodeProp.RELATION_NAME] = scanAndOperationsRegex[ScanAndOperationMatch.RelationName];
      if (scanAndOperationsRegex[ScanAndOperationMatch.Alias]) {
        this[NodeProp.ALIAS] = scanAndOperationsRegex[ScanAndOperationMatch.Alias];
      }
    } else if (bitmapRegex) {
    ...
    const parallelRegex = /^(Parallel\s+)(.*)/.exec(<string>this[NodeProp.NODE_TYPE]);
    if (parallelRegex) {
